context('Actions', () => {
  it('Get access to phone number', () => {
    cy.visit('http://automationpractice.com/index.php');
    cy.get('.shop-phone').contains('0123-456-789').click({ force: true });
  });
  ...
  it('Add new adress and go to the "shipping" step', () => {
    cy.contains('Add a new address').should('have.value', 'Add a new address').click();
    cy.get('input[type="text"]').eq(0).click().clear().type('Jakub');
    cy.get('input[type="text"]').eq(1).click().clear().type('Klimas');
    cy.get('input[type="text"]').eq(3).click().clear().type('Dluga 2');
    cy.get('input[type="text"]').eq(5).click().clear().type('Poznan');
    cy.get('#id_state').select('46');
    cy.get('input[type="text"]').eq(6).click().clear().type('57-540');
    cy.get('#id_country').select('21');
    cy.get('input[type="text"]').eq(7).click().clear().type('660039181');
  ...
  it('Agree to the terms and proceed', () => {
    ...
    it('Confirm the order', () => {
      cy.contains('I confirm my order').should('have.value', 'I confirm my order').click();
    });
  });
});
